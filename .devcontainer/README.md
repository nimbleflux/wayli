...
- **Node.js 25** (bookworm image) with **Bun**: for running the SvelteKit web application
- **PostgreSQL 18**: Database server (`ghcr.io/nimbleflux/fluxbase-postgres:18`)
- **Fluxbase**: Backend API service (port 8080)
- **Development Tools**: Git, GitHub CLI, PostgreSQL client, Deno (for edge functions), Claude Code

## Getting Started

1. Open this folder in VS Code
2. When prompted, click "Reopen in Container" (or run the command "Dev Containers: Reopen in Container")
3. Wait for the container to build and services to start — the post-create script installs web dependencies and syncs Fluxbase resources (`bun run sync:all`)
...
- **4000**: SvelteKit development server (web app)
- **8080**: Fluxbase API
- **5432**: PostgreSQL database

## Environment Variables

Secrets are auto-generated on first build by
`deploy/docker-compose/generate-keys.sh --stdout > .devcontainer/.env` (not
committed). The following are pre-configured:

- `FLUXBASE_PUBLIC_BASE_URL`: Fluxbase URL as seen from the browser (`http://localhost:8080`)
- `FLUXBASE_BASE_URL`: Fluxbase URL as seen from inside the containers (`http://fluxbase:8080`)
- `FLUXBASE_AUTH_JWT_SECRET`: JWT secret for authentication (auto-generated; change in production)
- `FLUXBASE_ANON_KEY`: Anonymous API key (development key only)
- `FLUXBASE_SERVICE_ROLE_KEY`: Service role API key (development key only)

**Note:** These keys are auto-generated development keys. They are safe for
...
```bash
cd web
bun install   # already run by post-create
bun run dev
```

The dev server listens on port 4000 (forwarded automatically).
...
If port 5432, 8080, or 4000 are already in use:
