...
When OSM tags are incomplete, search venue names. Restaurant names often contain:
- Cuisine keywords: "sushi", "pizza", "taco", "pho", "curry", "kebab", "wok"
- Dietary indicators: "vegan", "vegetarian", "plant", "green", "organic"
- Cultural words: "trattoria" (Italian), "izakaya" (Japanese), "bistro" (French)
- Activity hints: "golf", "tennis", "spa", "gym", "yoga"

Use ILIKE ANY for multiple patterns:
\`\`\`sql
-- Vietnamese food
geocode->'properties'->>'label' ILIKE ANY(ARRAY['%pho%', '%banh mi%', '%vietnamese%', '%saigon%', '%hanoi%'])

-- Italian restaurants
geocode->'properties'->>'label' ILIKE ANY(ARRAY['%pizza%', '%pasta%', '%trattoria%', '%italian%', '%ristorante%'])

-- Coffee/cafes
geocode->'properties'->>'label' ILIKE ANY(ARRAY['%coffee%', '%cafe%', '%espresso%', '%starbucks%', '%roaster%'])
\`\`\`
